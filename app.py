...
# SIDEBAR NAVIGATION
# ==========================================
st.sidebar.title("⚛️ Smart Fisika V2.2")
st.sidebar.markdown("---")
...
# ==========================================
# MENU 2: CHEAT SHEET (DENGAN TAMBAHAN RUMUS)
# ==========================================
elif menu == "Catatan Rumus & Cheat Sheet":
    st.title("📚 Kumpulan Catatan Rumus & Cheat Sheet")
    
    tab1, tab2, tab3, tab4 = st.tabs([
        "📊 Tabel Satuan Eksperimen", 
        "🌌 Konstanta Fisika Penting", 
        "📜 Rangkuman Hukum Fisika",
        "🧮 Kumpulan Rumus Praktis"
    ])
    ...
    with tab3:
        ...
        with st.expander("1. Hukum Newton (Mekanika)"):
            st.markdown("""
            *   **Hukum I Newton (Inersia):** $$\Sigma F = 0$$
            *   **Hukum II Newton:** $$F = m \cdot a$$
            *   **Hukum III Newton (Aksi-Reaksi):** $$F_{\\text{aksi}} = -F_{\\text{reaksi}}$$
            """)
            
        with st.expander("2. Hukum Fluida (Stokes & Archimedes)"):
            st.markdown("""
            *   **Hukum Stokes:** $$F_s = 6 \pi \eta r v$$
            *   **Hukum Archimedes:** $$F_a = \rho_f \cdot g \cdot V_{\\text{tercelup}}$$
            *   **Hukum Bernoulli:** $$P + \\frac{1}{2}\rho v^2 + \rho gh = \\text{Konstan}$$
            """)
            
        with st.expander("3. Hukum Termodinamika & Pemuaian"):
            st.markdown("""
            *   **Hukum Pemuaian Panjang:** $$\Delta L = L_0 \cdot \alpha \cdot \Delta T$$
            *   **Hukum I Termodinamika:** $$\Delta U = Q - W$$
            """)
            
        with st.expander("4. Hukum Kelistrikan (Coulomb & Ohm)"):
            st.markdown("""
            *   **Hukum Coulomb:** $$F = k \cdot \\frac{q_1 \cdot q_2}{r^2}$$
            *   **Hukum Ohm:** $$V = I \cdot R$$
            """)

    with tab4:
        st.subheader("🧮 Cheat Sheet Kumpulan Rumus Fisika")
        st.caption("Rumus matematika siap pakai untuk menyelesaikan problem fisika dasar.")
        ...
        with st.expander("🏃 2. Kinematika Gerak (GLB & GLBB)"):
            ...
            *   **Rumus Kuadrat Kecepatan (GLBB):** 
                $$v_t^2 = v_0^2 + 2a \cdot s$$
            """)
